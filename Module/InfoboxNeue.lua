...
--- https://starcitizen.tools/Module:InfoboxNeue
...
function methodtable.renderImage( self, filename )
	...
	if type( filename ) ~= 'string' and self.config.displayPlaceholder == true then
		...
		table.insert( self.categories,
			string.format( '[[Category:%s]]', 'Infobox using placeholder image' )
		)
	end
	...
	-- Check if multiple images are provided via comma-separated list
	if string.find( filename, ',' ) then
		local tabberData = {}
		local tabberCount = 0
		
		-- Split bằng %s*,%s* để tự động loại bỏ khoảng trắng dư thừa
		local images = mw.text.split( filename, '%s*,%s*' )

		for _, imgData in ipairs( images ) do
			local imgName, tabLabel = string.match( imgData, "^(.*):([^:]*)$" )
			
			-- Fallback if no label is provided
			if not imgName then
				imgName = imgData
				tabLabel = "Image " .. (tabberCount + 1)
			end

			imgName = mw.text.trim( imgName )
			tabLabel = mw.text.trim( tabLabel )

			-- Strip namespace if present
			local parts = mw.text.split( imgName, ':', true )
			if #parts > 1 then
				table.remove( parts, 1 )
				imgName = table.concat( parts, ':' )
			end

			local imgWikitext = tostring( mw.html.create( 'div' )
				:addClass( 'infobox__tabber-image-wrapper' )
				:wikitext( string.format( '[[File:%s|400px|frameless]]', imgName ) )
			)
			
			-- Prepare data table for Module:Tabber (label{n} and content{n})
			tabberCount = tabberCount + 1
			tabberData['label' .. tabberCount] = tabLabel
			tabberData['content' .. tabberCount] = imgWikitext
		end

		-- Load Module:Tabber and render
		self.modules.Tabber = self.modules.Tabber or require( 'Module:Tabber' )
		html:wikitext( self.modules.Tabber.renderTabber( tabberData ) )
	else
		-- Render single image
		...
		html:wikitext( string.format( '[[File:%s|400px]]', mw.text.trim( filename ) ) )
	end
...
--- @param data table The data containing icon or iconClass information
--- @return table The text wrapper element
local function renderItemIconElement( html, data )
	html:addClass( 'infobox__item--hasIcon' )
	
	local iconWrapper = html:tag( 'div' ):addClass( 'infobox__icon' )
	
	if data.iconClass then
		iconWrapper:tag( 'span' ):addClass( data.iconClass ):done()
	elseif data.icon then
		iconWrapper:wikitext( string.format( '[[Tập tin:%s|16px|link=]]', data.icon ) )
	end
...
function methodtable.renderItem( self, data, content )
	...
	local textWrapper = (data.icon or data.iconClass) and renderItemIconElement( html, data ) or html
...
function methodtable.renderInfobox( self, innerHtml, snippetText )
	...
	local function renderSnippet()
		...
		html:tag( 'div' )
			...
			:wikitext( string.format( '%s', 'Tóm tắt' ) )
			:done()
			:tag( 'div' )
...
function InfoboxNeue.new( self, config )
	local baseConfig = {
		...
		-- Placeholder Image
		placeholderImage = 'TheRealPlaceholder.png',
	}
